...
public class Main {
	...
	public static void main(String[] args) {
		...
		p[3] = new Zulieferer("Verkäufer 1");
		p[4] = new Verkäufer("Verkäufer 1");

		for (int i = 0 ; i < p.length; i++){
			if (p[i] instanceof INewsletter){ // Prüfen, ob das Objekt (vom Typ Person) auch vom Typ INewsletter ist
				INewsletter n = (INewsletter)p[i]; //wenn ja, kann es in den Typ gecastet werden
